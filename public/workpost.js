...
async function loadOrdersInQueue() {
  const response = await fetch(
    'https://Nest.gigantgiga.repl.co/order/res/queue',
  );
...
function setCheckboxEvents() {
  const checkBoxes = Array.from(
    document.getElementsByClassName('order-checkboxes'),
  );
...
async function loadQueueOrdersToSelect() {
  const selectElements = Array.from(
    document.getElementsByClassName('order-selection'),
  );
  ...
  selectElements.forEach((select) => {
    ...
    orders.forEach((order) => {
      ...
      const newOption = new Option(`${car} - ${order.number}`, order._id);
